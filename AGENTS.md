...
- Network + state logic is centralized in specific layers:
  ...
  - **Data**: `MoviesRepositoryImpl` orchestrates `data/local` and `data/external`, maps remote responses to domain entities, and delegates cache responsibilities to `LocalMoviesDataSource`.
...
- API contracts (`RemoteMovie`, `RemoteResult`, `RemoteMoviesDataSource` and `RemoteMovieDataSourceImpl`) live in `data/external`, mapping explicitly to `domain/entity/Movie.kt`.
